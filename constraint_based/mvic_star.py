...
class MVICStar(object):
    """
        ...
        related, e.g. Does A cause B? Is it possible that some unobserved cause
        relates A and B (unobserved confounding), etc.).

        This is an implementation of the IC* algorithm listed in Pearl's
        Causality (2009) book. However, the original algorithm assumed that
        data had no missingness. Running naive causal discovery algorithms,
        without adjusting for missing data, could lead to extraneous edges. It
        has been modified to account for missingness, by making use of ideas
        from the [Causal Discovery in the Presence of Missing Data (Tu et. al,
        ...
        we relax the causal sufficiency assumption by only having it apply to
        missingness indicators (i.e. all causes of missingness are (at least
        partially) observed).
        ...
        See [Causal Discovery in the Presence of Missing Data](http://proceedings.mlr.press/v89/tu19a/tu19a.pdf) for more details for the last 4 assumptions.
